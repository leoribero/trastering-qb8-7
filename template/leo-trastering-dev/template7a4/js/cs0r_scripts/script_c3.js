// Variable global para almacenar pricesC3
let pricesC3 = [];

// Función principal que maneja el fetch y el procesamiento de datos
async function initializePricesC3() {
    try {
        
        const response = await fetch("/scripts/data.json");
        const trKing = await response.json();

        // Verificar que trKing[3] existe y tiene las propiedades necesarias
        if (!trKing || !trKing[3] || !trKing[3].numBoutique || !trKing[3].disponibilites) {
            console.error('Datos no válidos en el JSON:', trKing);
            return;
        }

        // console.log("trKing.length", trKing.length);
        // console.log("trKing[3].disponibilites.length", trKing[0].disponibilites.length);
        const tamanos_disponibles = trKing[3].disponibilites.length;
        ...
        if (trKing[3].numBoutique == 3) {
            const cen_3_map = {};
            const cen_3 = [];

            trKing[3].disponibilites.forEach(data => {
                // Determinar planta
                ...
                // Construir nombre
                let productName = `${tipo}${num}_cen_3_${planta}_${sufijo}_men`;

                // Solo añadir si no existe aún
                if (!cen_3_map[productName]) {
                    // Llama a processProduct y añade el producto
                    const product = processProduct([data], [], planta, productName);
                    cen_3_map[productName] = product;
                }
            });

            // El array final:
            for (const key in cen_3_map) {
                if (cen_3_map[key]) cen_3.push(cen_3_map[key]);
            }

            pricesC3 = cen_3;
            window.pricesC3 = pricesC3;
            window.dispatchEvent(new CustomEvent('pricesC3Ready'));
            // Nuevo evento unificado para multi-centro
            window.dispatchEvent(new CustomEvent('pricesReady', { detail: { centro: 3, data: pricesC3 } }));
            console.log("pricesC3 cargado:", pricesC3);
        } else {
            console.error('No se encontró la boutique 3 en los datos');
        }
    } catch (error) {
...
function processProduct(productData, cen_3, floor, productName) {
    if (productData && productData.length > 0) {
        const data = productData[0];
        const name_cen_3 = data.nom;
