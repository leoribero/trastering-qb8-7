// Variable global para almacenar pricesC2
let pricesC2 = [];

// Función principal que maneja el fetch y el procesamiento de datos
async function initializePricesC2() {
    try {
                
        const response = await fetch("/scripts/data.json");
        const trKing = await response.json();

        // Verificar que trKing[1] existe y tiene las propiedades necesarias
        if (!trKing || !trKing[1] || !trKing[1].numBoutique || !trKing[1].disponibilites) {
            console.error('Datos no válidos en el JSON:', trKing);
            return;
        }

        // console.log("trKing.length", trKing.length);
        // console.log("trKing[1].disponibilites.length", trKing[0].disponibilites.length);
        const tamanos_disponibles = trKing[1].disponibilites.length;
        ...
        if (trKing[1].numBoutique == 2) {
            const cen_2_map = {};
            const cen_2 = [];

            trKing[1].disponibilites.forEach(data => {
                // Determinar planta
                ...
                // Construir nombre
                let productName = `${tipo}${num}_cen_2_${planta}_${sufijo}_men`;

                // Solo añadir si no existe aún
                if (!cen_2_map[productName]) {
                    // Llama a processProduct y añade el producto
                    const product = processProduct([data], [], planta, productName);
                    cen_2_map[productName] = product;
                }
            });

            // El array final:
            for (const key in cen_2_map) {
                if (cen_2_map[key]) cen_2.push(cen_2_map[key]);
            }

            pricesC2 = cen_2;
            window.pricesC2 = pricesC2;
            window.dispatchEvent(new CustomEvent('pricesC2Ready'));
            // Nuevo evento unificado para multi-centro
            window.dispatchEvent(new CustomEvent('pricesReady', { detail: { centro: 2, data: pricesC2 } }));
            console.log("pricesC2 cargado:", pricesC2);
        } else {
            console.error('No se encontró la boutique 2 en los datos');
        }
    } catch (error) {
...
function processProduct(productData, cen_2, floor, productName) {
    if (productData && productData.length > 0) {
        const data = productData[0];
        const name_cen_2 = data.nom;
