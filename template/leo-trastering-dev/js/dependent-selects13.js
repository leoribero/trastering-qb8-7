...
var handle_dependent_selects = function($parent) {
    ...
    if(parent_val) {
        ...
        for (i=0; i<$child.options.length; i++) {
            var $option = $child.options[i];
            if ($option.value == "") {
                $option.innerText = "Todos los tamaños";
                continue;
            }
...
function updateLink() {
	...
	var id_child=document.getElementById("id_child");
	// var listaComunidad=document.getElementById("listaComunidad");

	// if(id_child.selectedIndex>= 0 && listaComunidad.selectedIndex >= 0 && id_parent.selectedIndex >= 0){

	if(id_child.selectedIndex>= 0 && id_parent.selectedIndex >= 0){
		var centro = id_child.options[id_child.selectedIndex].value;
		// var comunidad = listaComunidad.options[listaComunidad.selectedIndex].value;
		var tamano = id_parent.options[id_parent.selectedIndex].value;

		// var a='http://asig/sele/'+centro+'/'+comunidad+'/'+tamano+'.html';

		var a='https://trastering.es/'+tamano+centro;
		document.getElementById("link").href = a;
	}
	if(id_child.selectedIndex!= ""){
		contN.classList.toggle('mostrar');
	}
}
