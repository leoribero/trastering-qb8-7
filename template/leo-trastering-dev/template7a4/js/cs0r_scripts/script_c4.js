// Variable global para almacenar pricesC4
let pricesC4 = [];

// Función principal que maneja el fetch y el procesamiento de datos
async function initializePricesC4() {
    try {
                
        const response = await fetch("/scripts/data.json");
        const trKing = await response.json();

        // Verificar que trKing[2] existe y tiene las propiedades necesarias
        if (!trKing || !trKing[2] || !trKing[2].numBoutique || !trKing[2].disponibilites) {
            console.error('Datos no válidos en el JSON:', trKing);
            return;
        }

        // console.log("trKing.length", trKing.length);
        // console.log("trKing[2].disponibilites.length", trKing[2].disponibilites.length);
        const tamanos_disponibles = trKing[2].disponibilites.length;
        ...
        if (trKing[2].numBoutique == 4) {
            const cen_4_map = {};
            const cen_4 = [];

            trKing[2].disponibilites.forEach(data => {
                // Determinar planta
                ...
                // Construir nombre
                let productName = `${tipo}${num}_cen_4_${planta}_${sufijo}_men`;

                // Solo añadir si no existe aún
                if (!cen_4_map[productName]) {
                    // Llama a processProduct y añade el producto
                    const product = processProduct([data], [], planta, productName);
                    cen_4_map[productName] = product;
                }
            });

            // El array final:
            for (const key in cen_4_map) {
                if (cen_4_map[key]) cen_4.push(cen_4_map[key]);
            }

            pricesC4 = cen_4;
            window.pricesC4 = pricesC4;
            window.dispatchEvent(new CustomEvent('pricesC4Ready'));
            // Nuevo evento unificado para multi-centro
            window.dispatchEvent(new CustomEvent('pricesReady', { detail: { centro: 4, data: pricesC4 } }));
            console.log("pricesC4 cargado:", pricesC4);
        } else {
            console.error('No se encontró la boutique 4 en los datos');
        }
    } catch (error) {
...
function processProduct(productData, cen_4, floor, productName) {
    if (productData && productData.length > 0) {
        const data = productData[0];
        const name_cen_4 = data.nom;
