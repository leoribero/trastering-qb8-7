...
async function initializePricesC10() {
    try {
        ...
        if (!trKing || !trKing[6] || !trKing[6].numBoutique || !trKing[6].disponibilites) {
        ...
        const tamanos_disponibles = trKing[6].disponibilites.length;
        ...
        if (trKing[6].numBoutique == 10) {
            ...
            trKing[6].disponibilites.forEach(data => {
                ...
                // Construir nombre
                let productName = `${tipo}${num}_cen_10_${planta}_${sufijo}_men`;

                // Solo añadir si no existe aún
                if (!cen_10_map[productName]) {
                    // Llama a processProduct y añade el producto
                    const product = processProduct([data], [], planta, productName);
                    cen_10_map[productName] = product;
                }
            });

            // El array final:
            for (const key in cen_10_map) {
                if (cen_10_map[key]) cen_10.push(cen_10_map[key]);
            }

            pricesC10 = cen_10;
            window.pricesC10 = pricesC10;
            window.dispatchEvent(new CustomEvent('pricesC10Ready'));
            // Nuevo evento unificado para multi-centro
            window.dispatchEvent(new CustomEvent('pricesReady', { detail: { centro: 9, data: pricesC10 } }));
            console.log("pricesC10 cargado:", pricesC10);
        } else {
...
function processProduct(productData, cen_10, floor, productName) {
    if (productData && productData.length > 0) {
        const data = productData[0];
        const name_cen_10 = data.nom;
